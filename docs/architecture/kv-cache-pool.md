...
- 每个 block 维护引用计数（被多少在途请求使用）。引用 >0 → 冻结迁移/驱逐（正确性约束）。
- 引用为 0 的 block 进入冷热排序，按**热度分**（f(访问频次, recency)，LFU-Aging）与容量阈值驱逐/下沉。
- **公共前缀** block（高复用）给予前缀亲和加权保护，驱逐时不易被选。
- 层间副本/移动、提升/下沉、主动迁移详见 [`storage-layer.md`](storage-layer.md) "冷热与生命周期管理"节；本节描述的驱逐是其中一环（被动兜底 + 主动 demotion 的终态）。
- 被驱逐但仍在对象存储（L4，永久权威）有副本的 block，可按需回填。
