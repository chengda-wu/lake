...
**(b) 改变了三方案的比较基础**:权威在控制面进程内存,etcd 只存降频 checkpoint。方案1/2 watch etcd 读到的是 **stale checkpoint**(落后于控制面内存的 live 视图),方案3 gRPC stream 推的是 **live 权威**。所以方案1/2 不再只是"工程成本"劣势,而是**一致性更差**——要读 live 权威只能直连控制面 stream,等于方案3 + 多一层 cgo,无收益。

| 方案 | 镜像来源 | 读到的是 | 结论 |
|---|---|---|---|
| 方案1 FFI | etcd watch | stale checkpoint | **否决**((b) 下读 stale,要 live 只能加 stream=方案3+cgo) |
| 方案2 直连 etcd | etcd watch | stale checkpoint | **否决**(同上 + 打破 etcd key space 隔离) |
| 方案3 gRPC stream | 控制面推送 | **live 权威** | **主方案(已选)**:唯一不限拓扑读 live 权威 |

> 方案1 的"机制统一"卖点(与计算 agent 共享 Rust mirror 代码)不成立:view mirror 是 agent 的**只读子集**(不搬 KV/不组装 block table/不 RDMA),共享的只是"订阅 stream + apply 增量 + 去重 + gap replay"协议逻辑,那套 proto+apply Go 一样能共享。为这点可移植逻辑付 cgo 交叉编译 + panic 跨边界,不划算。

### 新方案:方案甲 共享内存镜像(同机部署优化,已采纳)
...
控制面进程内存(权威) ──写入──> 共享内存段(seqlock) <──映射── Router 进程
                          纯内存读写,零拷贝零序列化
跨机 Router 副本 ──gRPC stream──> 控制面(落回方案3)
```

- **与 (b) 协同最强**:权威本就在控制面进程内存,共享内存只是把那块内存的**只读视图**暴露给同机 Router,连"推"都省了。
- **前提**:控制面与 Router 同机(lake 控制节点上常成立)。**跨机 Router 副本用不了**,落回方案3。所以它不是独立方案,是"同机替 stream、跨机仍 stream"的**分层混合**。
- **代价**:共享内存 schema 要内存布局稳定(改字段走版本号)。同机是优化前提,使 #3 待讨论 #1(控制面与 Router 是否同机)对 #4 变关键。

### 其余机制(已考虑,否决/备选)

- **方案乙 周期快照拉取**:Router 每 X ms 拉全量快照替换镜像,无增量/gap/replay,协议极简。代价:新鲜度受固定间隔限制 + 全量带宽浪费。**备选**(镜像小/要求极简的早期)。
- **方案丙 pub/sub(NATS/Redis)**:dynamo 的做法(`distributed.rs:474`)。lake 单权威引入 broker 只多依赖与一致性问题。**否决**——多发布者场景才需要。
- **方案丁 long-poll / pull-delta**:stream 的 pull 变体,非独立方案。

### 粒度与协议(答 #4 待定)

1. **粒度 = 增量事件 + 单流序号 + gap replay(非快照)**。(b) 下控制面是**单权威、单发布者**,`publisher_id` 恒为控制面,退化为单流 sequence(比 dynamo 多发布者简单)。借鉴 `DeduplicatingStream`(`mod.rs:218`)的 `(publisher_id, sequence)` 去重。冷启动:snapshot-on-connect(带序号建镜像)→ 之后增量。断线:Router 报 "resume from seq N",控制面重放。
...
4. **etcd checkpoint 三重角色**(非浪费):控制面重启重建内存权威 / Router 冷启动 snapshot 源 / stream 断时 Router 回退(更陈旧但非空)直到重连。这让方案3"依赖控制面在线"有了兜底——(b) 与方案3 协同。
5. **陈旧只损性能**。dynamo gap/replay 保证最终一致,期间 router 误判 → 回退确认 → 回填,正合 lake `consistency.md` §1"陈旧只损性能不损正确性"。

### 剩余风险(归 #3,非 #4)

方案3 依赖控制面在线 → **控制面单点**。归 #3 待讨论 #4(单 leader + etcd,多副本留 P7)。#4 声明"假设控制面 HA 由 #3 解决"。
...
### #4 结论速查(Router 持位置视图镜像)

- **方案3 gRPC stream(主)**:唯一不限拓扑读 live 权威。
- **方案甲 共享内存(同机优化)**:控制面与 Router 同机时替 stream(零拷贝零序列化),跨机落回方案3。
- **方案1/2 否决**:(b) 下 watch etcd 读 stale checkpoint,要 live 只能加 stream=方案3+cgo(方案1)/打破 key space 隔离(方案2)。
- **方案乙 周期快照**:备选(早期/小镜像);**方案丙 pub/sub / 方案丁 long-poll**:否决/非独立。
- **粒度**:snapshot-on-connect + 增量 + 单流序号(单权威,`publisher_id` 退化) + gap replay(bidi stream 带 resume,非单独 RPC)。
- **边4=边5**:同协议不同订阅者。
- **etcd checkpoint 三重角色**:控制面重启重建 / Router 冷启动 snapshot / stream 断回退。
- **剩余风险(归 #3)**:控制面单点,HA 由 #3 待讨论 #4 解决。
