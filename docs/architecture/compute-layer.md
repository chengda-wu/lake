...
| 里程碑 | 内容 | 主改 | 验收 |
|--------|------|------|------|
| **C11** | **Runner 静态 buffer + lake AttentionMetadata v2**：引入固定地址 `InputBuffers` / block table / slot mapping 镜像；`InputBatch` 保持 host 权威但能导出 device-ready 形态 | `lake/engine/input_batch.py`、`lake/engine/model_executor/layers/attentions/attention_metadata.py`、`lake/engine/model_runner.py`、`lake/engine/pool_types.py` | 单测覆盖 ragged query_start、decode 单 token、chunked extend、agent block table 缺失/错配 |
