...
| # | 缺口 | 现状 | 建议落点 |
|---|------|------|----------|
| D1 | **`SchedulerOutput` / `NodeScheduleOutput` 字段草图** | 只定了"类 vLLM、由 node_scheduler 产出";未定 forward mode 枚举、read/write set、请求增删、与 Router Dispatch 的字段映射;**DP sync 后写入的 `global_num_tokens` 等**(见 [`scheduling.md`](scheduling.md) §3.1)一并纳入 | 字段全集与架构差异见 [`../research/scheduler-worker-interface.md`](../research/scheduler-worker-interface.md);落点本节扩写或 `scheduling.md` §3 |
...
| D3 | **角色配置 schema** | `role=prefill\|decode\|hybrid` 已定方向;未定完整启动配置(模型、TP、是否挂 drafter、arena 尺寸、上报指标标签) | `runtime` 配置节;与冷启动 Warm 对齐 |
...
| D5 | **节点级 scheduler 与 agent 的交互序** | 方案 Z:读视图组 batch、缺失补拉;未定 schedule 一步内"补拉等待预算 / 是否允许部分命中进 batch / 与 ready fence 谁先谁后" | `scheduling.md` §3 + 本节 step 热路径细化 |
