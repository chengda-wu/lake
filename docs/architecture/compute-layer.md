...
- SGLang 据 `draft_arch` 判定(如 `Step3p5MTP` 走 chain);chain 需 `post_forward` 保留更全的 hidden states(FULL),non-chain 只需 LAST——直接影响 hidden states 暂存量(见下"hidden states 暂存")。
- chain-style 的层间链式传播使 `pre_forward` 内部状态依赖更复杂(每步输出喂下一步),暂存物不止"target 末 token hidden states",还含各中间层的输出;接入多层 MTP 时按 chain/non-chain 区分暂存策略。
...
drafter 需 target **最后 `num_mtp_layers` 个 token 的 hidden states**作自回归输入(单层 / non-chain 多层:LAST;chain-style 多层:FULL,见上"多层 MTP"):

- 按 token 组织、每步滚动保留最近 `num_mtp_layers` 个 token;采用 **r-type 的紧凑存储形态**(per-request,不随序列线性堆积)。chain-style 多层 MTP 需保留更全的 hidden states(FULL)+ 各中间层输出,暂存量更大,仍属 L0 r-type、不进池。
- 仅 L0(HBM)暂存,每步滚动覆盖;**不进 radix、不落 L1+**(跨请求无复用价值,故不涉及全前缀复用)。
- 同 step 与 KV 一同产出,但 KV 写回池(容错 + 前缀生长),hidden states 用完即滚,生命周期独立。block 组织与命中策略见 [`kv-cache-pool.md`](kv-cache-pool.md) "draft 中间态"。
...
- **多层 MTP chain / non-chain**:接入多层 MTP 时的 hidden states 暂存策略(chain 需 FULL + 中间层输出,non-chain 只需 LAST)与 `speculative_num_draft_tokens == num_layers` 等约束对齐。
