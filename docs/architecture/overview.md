...
```
**层 = 介质,不是位置。** 四层全部由存储池统一管理(放置/驱逐/副本/冷热/生命周期)。L0/L1 做缓存副本(易失),L2/L3 间按移动,L3 永久权威;L2(NVMe)= F4 恢复点(NPU 故障与本机 NVMe 解耦),L3 = SSOT(抗整机级/池级失败)。DRAM/NVMe 各是一层、一个池,block 放本机还是远端是池的放置决策(同 L0 放哪个节点 HBM),不按位置拆成两层。冷热按引用数+热度分(LFU-Aging)+前缀亲和判定;迁移主动为主+被动兜底。详见 [`storage-layer.md`](storage-layer.md)。
