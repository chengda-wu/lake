...
> **物理约束**：L0–L2 统一管理是**元数据/逻辑层面**的统一（谁在哪个物理介质、何时驱逐、副本关系），物理数据访问仍受 GPU 本地性约束——attention kernel 读 KV 必须在本机 HBM，无法高效跨节点直接读他人 HBM。因此存储池的 L0 放置决策必须保证"被同一 batch 引用的 KV 放在同一 GPU HBM"，这实质上把部分 batch 调度职责与存储池的放置决策耦合（P1 `execution-modes.md` / `consistency.md` 详定边界）。
