...
> **Worker / ModelRunner / 投机解码 / 与 vLLM 对照(dummy、DP/TP/PP) / DP 双层管理(引擎 Controller + Gateway cache_aware)**见 [model-runner.md](model-runner.md);block 何时释放/彻底放弃见 [block-lifecycle.md](block-lifecycle.md);上游 issue/roadmap 痛点整理见 [pain-points.md](pain-points.md)。
