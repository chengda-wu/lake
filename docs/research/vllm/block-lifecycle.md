...
**KV / block 释放在 Scheduler 的 `update_from_output`(处理完本步 `ModelRunnerOutput` 之后)；Worker 侧另清 runner 缓存态。** 与 SGLang「同在 Scheduler CPU 收尾」类似，但 vLLM 因 Scheduler↔Worker **跨进程**，把「放 KV」和「清 Worker RequestState」拆成两端。

```
Scheduler.schedule() → SchedulerOutput
  → Worker.execute_model → ModelRunnerOutput
  → Scheduler.update_from_output(output)     ← 判定 stop + 释放 KV 在这里
       check_stop / _handle_stopped_request
       → _free_request → _free_blocks → KVCacheManager.free
       → finished_req_ids.add(req_id)         # 写入下一步 SchedulerOutput
  → (下一步) Worker.finish_requests(finished_req_ids)  ← 只清 runner 态，不再 free KV
```

| 端 | 阶段 / 符号 | 释放什么 |
|----|-------------|----------------|
| **Scheduler** | `update_from_output` → `_free_request` / `_free_blocks` | **KV blocks**（`KVCacheManager.free` → `ref_cnt-=1`）；encoder cache；connector `request_finished` |
| **Scheduler** | 外部 abort：`finish_requests` | 同上，API 断开等 |
| **Worker** | 下一步 `execute_model` 开头 `finish_requests(scheduler_output)` | **RequestState / input 缓存**（`_remove_request`），**不是** BlockPool |
| **Worker** | `free_states` | `free_encoder_mm_hashes` 对应的 encoder cache 条目 |

要点：

1. **KV 权威在 Scheduler 进程**（与 `KVCacheManager` / `BlockPool` 同进程）——`ModelRunner` 不调用 `free_blocks`。
2. `SchedulerOutput.finished_req_ids` 是给 Worker 的**状态回收通知**，不是 KV free 指令（KV 已在 Scheduler 侧放下）。
3. Async / connector 可 `delay_free_blocks`：先记 finished，等 KV transfer / remote KVs 完成再 `_free_blocks`。

> **对照 SGLang**：SGLang Scheduler 与池同进程，`process_batch_result` 里一次性 `release_kv_cache`，**无**「下一步再通知 Worker 放 KV」；Worker/Runner 几乎无请求级 KV 账本。见 [../sglang/block-lifecycle.md](../sglang/block-lifecycle.md)「请求结束的调度阶段」。

#### `_free_request` 做什么

请求收尾走 `Scheduler._free_request` → `_free_request_blocks` → `KVCacheManager.free(request)`:
...
**异步路径**(`defer_block_free` / deferred frees):不立即 free,攒到 `deferred_frees`,由 `_drain_deferred_frees` 批量 `free_blocks`。**这条路径有 bug**:`pop_blocks_for_free` 把多 group 的块 flatten 成一个 list 再整体 reverse,丢了 per-group 驱逐顺序(#48489,见 [pain-points.md](pain-points.md) 1.5)。

> **对照 lake**:lake「请求结束 ref 归 0→可驱逐候选→驱逐覆写才摘位置视图」与 vLLM 同构;vLLM 的 deferred-free 是性能优化但引入顺序 bug,lake 用引用计数更显式。请求结束的**阶段**上 lake 更接近「Scheduler/节点 scheduler + 池 agent」协作,而非 Worker 自持 BlockPool。
...
| 调度器收尾(判定 stop + 放 KV) | `vllm/v1/core/sched/scheduler.py`::`update_from_output` / `check_stop` / `_handle_stopped_request` |
| 调度器请求释放 | `scheduler.py`::`_free_request` / `_free_blocks` / `_free_request_blocks` / `finish_requests`(外部 abort) |
| Worker 清 runner 态 | `vllm/v1/worker/gpu/model_runner.py`::`finish_requests` / `free_states`（吃 `SchedulerOutput.finished_req_ids`） |
