...
| DualPath 设计 | 我们对应 | 说明 |
|---------------|----------|------|
| **双网络隔离**:CNIC(计算)/ SNIC(存储)物理分离,KV 传输走 CNIC 不干扰模型通信 | 我们的 compute network(L0→L0 RDMA 数据面)/ storage network(L2/L3/L4 访问)分离 | 我们的池统一管理,NIC 带宽是池的资源;DualPath 的"借 decode SNIC"在我们这里=池可选从哪个节点的存储带宽加载 + 经 compute network 中转。见 [`../architecture/kv-cache-pool.md`](../architecture/kv-cache-pool.md) "双网络路径" |
...
- **分层**:DualPath 聚焦 storage↔HBM 的加载路径,无 L0–L4 统一分层/冷热/多模型配额;我们全层归池统一管理。

## 我们"原生支持"的含义

DualPath 的双路径(用哪条 NIC / 从哪个引擎加载 KV)在我们架构里**已具备物理基础,不新增流类型**:
- **storage-to-prefill** = P 侧从 L3/L4 经 storage network 加载(补拉)。
- **storage-to-decode + CNIC 回传** = D 侧从 L3 经 storage network 加载 + 经 compute network 传 P(D→P)。
