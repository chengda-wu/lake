...
- **Mooncake 做不了 DualPath（根因 = dumb blob）**:DualPath 的 D→P 直传要池判断三件事——①这段延伸 KV 是哪个前缀的延伸（前缀身份）②下个 prefill 需要哪段（前缀匹配）③这段是否已在 D 的 HBM、能否零存储读取直传（本地命中判定）。三件都要**懂前缀结构**。Mooncake store 是 dumb blob（`tenant+key` 存不透明字节块，无 radix/内容寻址），三件都做不了——它只能"把指定 key 的字节从 A 搬到 B"，而"搬哪个、能否直传"的判断无人能做（Mooncake 不懂前缀，引擎在 lake 又零地址）。这与它做不了前缀复用、做不了 D-direct 是同一个根源：不懂 KV 内部结构。lake 靠 radix（懂前缀）+ 位置视图（懂位置）支撑 DualPath；Mooncake 仅可借 transfer-engine 做最底层的字节搬运。
