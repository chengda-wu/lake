...
- **执行模型**：drafter(MTP/EAGLE 一类)默认与 decode(target)共置、同 step 串行(drafter 在 target 之后,draft token 放下一轮 target 之前);MTP 重算产出 `1 + num_mtp_layers` 个 token,残差 prefill 短时需左 pad;drafter 的 hidden states 仅 L0 暂存、不进池。详见 [`../architecture/compute-layer.md`](../architecture/compute-layer.md) "投机解码"节。
