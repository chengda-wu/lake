...
- **多租户隔离（远期预留，当前不实现）**：多租户隔离归外部控制面/部署切分（按 model_id 命名空间或独立集群），lake 当前不做 KV 租户隔离——同 model_id 内全局共享复用。未来若实现，靠 `KVBlockID` 加 scope 维度过滤隔离（见 [`features.md`](features.md) F8、[`../architecture/kv-cache-pool.md`](../architecture/kv-cache-pool.md) "Block 寻址"预留）。
...
- **成本归因**：按 model_id 命名空间/部署归因算力与 KV 存储消耗（多租户级归因属外部计费，lake 不做）。
...
- F1 前缀复用 → 必须满足"可观测性（命中率 metrics）"+"安全（model_id 命名空间隔离）"。
