...
- [x] 分层缓存引擎（RAM/NVMe，对象存储回填）（P4.3：`LocalTierEngine` L0–L3 内存站位 + `TierPipeline`/`BandwidthPool` + PutEnd/WRITEBACK/`RequestBarrier` + 命中率·成本曲线单测 + criterion bench；真 NVMe/对象存储介质与跨机 defer P5）
