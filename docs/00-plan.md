...
> **代码级复用（两模块正交，P3 尚未链依赖）**：  
> - **A** Mooncake transfer-engine → `rust/transfer`（字节搬迁）  
> - **B** Dynamo kvbm-logical（fork 抽 crate）→ `rust/kv-pool` + `controlplane`（radix / 分层块管理）  
> 详见同文档「代码级复用策略（按模块，互不替代）」；其余以设计借鉴为主。

- [ ] 内容寻址 block 存储 + 引用计数 + LRU 驱逐（复用 B 起步）
...
- [ ] Python worker 接入真实（小）模型 + Triton kernel（落地 `python/{engine,runtime}` 实现树，废止 `prefill/decode/draft` 空壳）
- [ ] 同步工程基建：`.github/workflows/build.yml` 的 import 校验与 `python/pyproject.toml` 的 `packages.find` 切到 `engine`/`runtime`/`kernels`（避免仍校验旧三包）
