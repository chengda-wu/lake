...
- [x] 内容寻址 block 存储 + 引用计数 + LFU-Aging / 前缀亲和驱逐（复用 B 起步；P4.2：`ReportRef` + 仅 pub `MultiLruBackend`≈LFU-Aging / `LineageBackend`≈前缀亲和；不 pub 纯 Lru/Fifo，见 `rust/vendor/UPSTREAM.md`）
