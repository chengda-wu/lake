...
| 点 | P3 | 生产（P4+） |
|----|----|-------------|
| Gateway | **可选**；冒烟直接 curl Router OpenAI HTTP（边2） | Bifrost → Router |
| KV 字节 | `SkeletonKvService` **gRPC 传 bytes**（入 Rust 内存池） | RDMA 旁路（边7/8），proto 仅控制信令 |
| worker↔agent | Router **直调** `WorkerService.Generate`；worker 内调 ControlPlane+SkeletonKv | Dispatch(边10)→ agent → FFI(边6) |
...
- [x] Bifrost(Gateway) 接收请求 → Router 路由（P3：**curl 直打 Router**；Bifrost 配置样例可后补）
- [x] Go Router OpenAI `/v1/chat/completions` → gRPC `WorkerService.Generate`
- [x] Python mock worker：LookupPrefix → SkeletonKv Get/Put → RegisterBlocks → mock decode
- [x] Rust `controlplane` + `kv-pool` 进程（内存权威 + 内存 bytes）
- [x] 端到端冒烟：两请求共享前缀，第二个 `reused_blocks>0`（`./deploy/smoke.sh`，替代 `python -m src`）
- [x] `deploy/run-local.sh` 一条命令起全栈，curl 打通

**完成判据**：`deploy/` 一条命令起全栈，curl 打通；共享前缀第二次请求命中 KV。✅（`feat/p3-min-skeleton`）

---

## P4 — KV Pool 原型（Rust）

> 实现参考:`3rdparty/` 五个 submodule(SGLang HiCache / Mooncake / LMCache / vLLM / Dynamo),逐层对应与借鉴顺序见 [`research/3rdparty-reference.md`](research/3rdparty-reference.md) "实现参考顺序建议"。
...
这三件是后续一切编码的前提。P2 中技术选型已定（存储+存储控制面 Rust / 请求控制面 Go / 计算 Python+Triton），重点是把接口边界和目录结构定稿。
