...
- [x] 客户端 → Go Router OpenAI `/v1/chat/completions`（**无 Gateway/Bifrost**）→ gRPC `WorkerService.Generate`
...
**现在做**：P3 最小可运行骨架（[PR #19](https://github.com/chengda-wu/lake/pull/19)）收尾 → **P4 KV Pool 原型**。

P0–P2 已完成（特性 / 架构 / 三语言空壳+proto）。P3 验证跨语言联通与前缀复用冒烟；入口直打 Router（无 Bifrost）。
