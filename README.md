...
```
 ...
 └───────────────────────────────────────────┬───────────────────────────────────────────┘
  ...
  存储池的四层是同一个池的四个 tier(层=介质:HBM→DRAM→NVMe→对象存储,逐 tier 池化、
  热冷 promotion/demotion)。DRAM/NVMe 各是一层、一个池,block 放本机还是远端 KV Node
  由池放置决定(同 L0 放哪个节点 HBM),不按位置拆层。L2(NVMe)=F4 恢复点(NPU 故障与
  本机 NVMe 解耦)、L3=SSOT。物理载体分布不同但元数据全归存储池统一管理,计算节点不拥有任何一层。
