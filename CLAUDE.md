...
1. **混合执行模式**：请求不固定走 P→D。Router 按 `f(请求, 集群状态) → (模式, 节点)` 在三种模式间逐请求选路：
   - **PD 分离** / **混部**（同节点完成）/ **D-direct**（APC 命中直跳）。
   - **关键区分**：Pool 命中（省重算但需传输）≠ APC 命中（D 节点本地，可 D-direct）。
   - 混部与 D-direct **不违背存算分离**——KV 仍写回 Pool；分离的是"存储与计算"，不是"P 与 D 必须分机"。
...
SLO 数值是 draft（待 P7 校准），但**约束关系是硬的**：TTFT/ITL/冷启动等目标倒逼架构取舍（如 D-direct 模式选择开销 < 5ms，否则吃掉 APC 收益）。
