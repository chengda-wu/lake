...
func TestSelectExecModePoolHitPD(t *testing.T) {
	...
	if got := SelectExecMode(h, 16, RolePrefill); got != ExecModePDDisagg {
		t.Fatalf("prefill role = %s, want PD_DISAGG", got)
	}
	if got := SelectExecMode(h, 16, RoleDecode); got != ExecModePDDisagg {
...
func TestSelectExecModeLocalHitDDirect(t *testing.T) {
	...
	for _, role := range []WorkerRole{RoleHybrid, RolePrefill, RoleDecode} {
		if got := SelectExecMode(h, 16, role); got != ExecModeDDirect {
...
func TestSelectExecModeColocated(t *testing.T) {
	if got := SelectExecMode(PrefixHint{}, 8, RoleHybrid); got != ExecModeColocated {
	...
	if got := SelectExecMode(h, 16, RoleHybrid); got != ExecModeColocated {
		t.Fatalf("pool hit + hybrid = %s, want COLOCATED", got)
	}
	// 边界:空 prompt / 零 computed 不触发 D-direct
	if got := SelectExecMode(PrefixHint{LocalHit: true}, 0, RoleHybrid); got != ExecModeColocated {
